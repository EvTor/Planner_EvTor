...
const EventForm = ({ users,
    ...
    const matchTypedUser = () => {
        const matchedUsers = users.filter(user => {
            if (user.email === emailForMatching) { return user }
        });
        if (Array.isArray(matchedUsers) && matchedUsers.length) { setNewMember(matchedUsers[0]) }
    ...
    const addMemberToList = () => {
        ...
        if (listOfMembers.filter(member => member.id === newMember.id).length > 0) { return false }
    ...
    const addFriendToList = (friend) => {
        if (listOfMembers.filter(member => member.id === friend.id).length > 0) { return false }
